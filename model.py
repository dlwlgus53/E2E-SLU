...
class T5Gen_Model(nn.Module):
    ...
    def batch_generate(
        ...
        if self.add_special_decoder_token:
            if generate_mode == "bs":
                start_token, end_token, start_token_id, end_token_id = (
                    "<sos_b>",
                    "<eos_b>",
                    self.sos_b_token_id,
                    self.eos_b_token_id,
        ...
        origin_outputs = self.model(
            input_ids=src_input, attention_mask=src_mask, decoder_input_ids=src_input
        )
        ...
        for predicted_ids, logit in zip(outputs, max_logit):
            ...
            # print (one_res_text)
            one_res_text = (
                one_res_text.split(start_token)[-1].split(end_token)[0].strip()
            )
            ...
            try:
                ...
                first_idx = 0
            try:
                last_idx = (predicted_ids.cpu() == 32100).nonzero(as_tuple=True)[0][
                    0
                ] + 1
            except:
