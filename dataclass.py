...
class E2Edataclass:
    def __init__(self, text_tokenizer, text_data_path, audio_file_list, data_type,  short = 0):
    ...
    def raw_data_to_list(self, dataset):
        ...
        for d_id in dataset.keys():
            S +=1
            if self.short == True and S > 100: break
            dial_num +=1
            dial = dataset[d_id]
    ...
    def __getitem__(self, index):
        ...
        if turn_id>0: # TOOD careful about this
            sys_audio_path = self.audio_path_dict[dial_id][turn_id-1]['system']
        else: # It should be None because system has information about the answer
            sys_audio_path = None

        return {"question": question, "target": target, "turn_id" : turn_id,"dial_id" : dial_id,\
            'sys_audio_path' : sys_audio_path, 'user_audio_path' : user_audio_path}
    ...
    def collate_fn(self, batch):
        ...
        user_audio_path = [x["user_audio_path"] for x in batch]
...
if __name__ == '__main__':
    ...
    test_dataset = E2Edataclass(text_tokenizer, args.text_test_data_path, args.audio_test_file_list, 'test',  short = 1)



    test_data_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=16, collate_fn=test_dataset.collate_fn)
